...
export default function App() {
  ...
  const action = (a) => {
    if (window.AndroidBridge) {
      ...
      if (a === 'CLOSE' || a === 'REJECT') setIsIncoming(false);
    }
  };

  // Funzione solo per test nel browser
  const simulateCall = () => {
    setNum('+39 347 123 4567');
    setIsIncoming(true);
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-transparent p-4 font-sans">
      
      {/* Mostra il simulatore solo se non siamo su Android e non c'è una chiamata */}
      {!window.AndroidBridge && !isIncoming && (
        <button 
          onClick={simulateCall}
          className="bg-slate-800 text-white px-6 py-3 rounded-full font-bold shadow-lg animate-bounce"
        >
          Simula Chiamata in Arrivo 📞
        </button>
      )}

      {/* Overlay Card (visibile solo quando arriva una chiamata o simulazione) */}
      {isIncoming && (
        <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.3)] overflow-hidden border border-slate-100 relative animate-in fade-in zoom-in duration-300">
          <button 
